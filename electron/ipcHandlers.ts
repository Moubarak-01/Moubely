...
export function initializeIpcHandlers(appState: AppState): void {
  ...
  // Content Dimensions (Auto - kept for backward compatibility if needed)
  ipcMain.handle(
  ...
  ipcMain.handle("chat-with-image", async (event, { message, imagePath }) => {
    try {
      const result = await appState.processingHelper.getLLMHelper().chatWithImage(message, imagePath)
      return result
    } catch (error: any) {
      console.error("Error in chat-with-image handler:", error)
      throw error
    }
  })
  ...
  // Mouse Event Ignoring (Click-Through)
