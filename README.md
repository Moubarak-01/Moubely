...
- 📜 **Scroll Portal (Ghost Interaction)**: Dynamic scrolling using Ctrl + Up/Down for seamless history navigation in Private Mode
...
| **Interaction Layer**   | Manual Toggles           | **Scroll Portal** (Atomic Ctrl+Up/Down Interaction)            |
...
Implemented a highly specialized **Input Passthrough** mode (`Shift + A`). While the window becomes 100% transparent to system-level clicks for underlying workflow access, our **Scroll Portal** logic allows users to navigate chat history temporarily using `Ctrl + Up` and `Ctrl + Down` for seamless scrolling without exiting Private Mode.
...
**Solution:** Developed a **Modifier-Driven Scroll Override**. By leveraging Electron's global shortcuts and WebContents IPC messaging, we intercepted `Ctrl + Up` and `Ctrl + Down` keys directly at the system level. This allows users to inject scroll wheel events seamlessly into the transparent, unclickable UI without ever needing to solidify the window.
